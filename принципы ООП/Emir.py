class Person:
    def init(self, name, sex, race, position):
        self.name = name
        self.sex = sex
        self.race = race
    ...
    def description(self):
        print(
            f'Имя персонажа - {self.name}, пол - {self.sex}, раса - {self.race}, позиция - {self.position}, рост - {self.weight}, вес - {self.height},'
            f'здоровье - {self.health}, урон - {self.damage}, защита - {self.defence}')


class Human(Person):
    def init(self, name='Adam', sex='male', race='human', position='earth'):
        super().init(name, sex, race, position)
        self.weight = 180
        self.height = 85
        self.health = 1000
        self.damage = 140
...
class Orc(Person):
    def init(self, name='Ultazar', sex='male', race='orc', position='drenor'):
        super().init(name, sex, race, position)
        self.weight = 220
        self.height = 140
        self.health = 800
